...
export const Toolbar: FC<ToolbarProps> = ({ editor, onOpen, onNew, onSave }) => {
  ...
  const { currentFontFamily, currentFontSize, currentColor, currentHighlight, headingFmt } = useMemo(() => {
    if (!editor) {
      return { currentFontFamily: '', currentFontSize: '', currentColor: '#000000', currentHighlight: '#ffff00', headingFmt: '' }
    }
  ...
  return (
    <Box sx={{ 
      ...
      <Divider orientation="vertical" flexItem sx={{ mx: 0.5, opacity: 0.5, height: '60%' }} />
      ...
      {/* Text formatting */}
      <ToggleButtonGroup size="small" sx={{ '& .MuiToggleButton-root': { px: 0.75, py: 0.25, border: 'none', fontSize: 14, '&:hover': { backgroundColor: 'action.hover' }, '&.Mui-selected': { backgroundColor: 'action.selected' } } }}>
      ...
      {/* Headings */}
      <ToggleButtonGroup size="small" sx={{ '& .MuiToggleButton-root': { px: 0.75, py: 0.25, border: 'none', fontSize: 12, fontWeight: 600, '&:hover': { backgroundColor: 'action.hover' }, '&.Mui-selected': { backgroundColor: 'action.selected' } } }}>
      ...
      {/* Lists */}
      <ToggleButtonGroup size="small" sx={{ '& .MuiToggleButton-root': { px: 0.75, py: 0.25, border: 'none', '&:hover': { backgroundColor: 'action.hover' }, '&.Mui-selected': { backgroundColor: 'action.selected' } } }}>
      ...
      {pendingCount > 0 && <Chip label={`${pendingCount} pending`} size="small" color="primary" variant="outlined" sx={{ fontSize: 10, height: 20 }} />}
      ...
      <TTip title="AI Writing Assistant (Ctrl+K)"><IconButton size="small" onClick={() => useAppStore.getState().setAIAssistantOpen(!useAppStore.getState().aiAssistantOpen)}><AutoAwesomeIcon sx={{ fontSize: 17 }} /></IconButton></TTip>
